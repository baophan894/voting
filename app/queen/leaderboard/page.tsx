    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          ...
          {menuOpen && (
            <div className="md:hidden absolute top-full left-0 right-0 bg-slate-900 border-b border-slate-700 p-4 space-y-2">
              ...
              <Link href="/king/leaderboard" className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-800 transition-colors">
                <Trophy className="w-5 h-5 text-yellow-400" />
                <span className="font-medium text-slate-200">King Leaderboard</span>
              </Link>
            </div>
          )}
        </div>
      ...
      <section className={`pt-24 pb-8 px-4 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-slate-400">Queen - Year End Party 2026</p>
        </div>
      </section>
            ...
            {rest.map((candidate, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 bg-pink-500/20 px-4 py-2 rounded-full">
                  <p className="text-pink-400 font-bold text-lg">{candidate.votes}</p>
                  <span className="text-pink-400/60 text-xs">votes</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Footer with background image hint */}
      <footer className="relative bg-slate-900 text-white py-8 px-4 overflow-hidden">
        <div className="absolute bottom-0 right-0 w-64 h-48 opacity-20">
          {/* Placeholder for ship image like reference */}
        </div>
